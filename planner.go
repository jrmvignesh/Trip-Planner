...
func createLoc(rw http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	c := uc.session.DB("cmpe273").C("users")
	...
	uc.session.DB("cmpe273").C("users").Insert(R)
	UJ, _ := json.Marshal(R)
	fmt.Fprintf(rw, "%s", UJ)
...
func getLoc(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	...
	if err := uc.session.DB("cmpe273").C("users").FindId(id).One(&V); err != nil {
		w.WriteHeader(404)
		return
	}
	fmt.Println(V)
...
func putLoc(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	...
	if PutRequestIndex < len(V.Bestlocation_ids) {
		...
		if err := uc.session.DB("cmpe273").C("users").FindId(oid).One(&M); err != nil {
			w.WriteHeader(404)
			return
		}
		...
		PutRequestIndex++
		if err := uc.session.DB("cmpe273").C("users").Update(V, V_Updated); err != nil {
			w.WriteHeader(404)
			return
